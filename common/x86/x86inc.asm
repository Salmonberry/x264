...
%macro WIN64_PUSH_XMM 0
    ...
    %if xmm_regs_used > 6 + 16*cpuflag(avx512)
        movaps [rstk + stack_offset +  8], xmm6
    %endif
    %if xmm_regs_used > 7 + 16*cpuflag(avx512)
        movaps [rstk + stack_offset + 24], xmm7
    %endif
    %assign %%xmm_regs_on_stack xmm_regs_used - 16*cpuflag(avx512) - 8
...
%macro WIN64_SPILL_XMM 1
    ...
    ASSERT xmm_regs_used <= 16 + 16*cpuflag(avx512)
    %assign %%xmm_regs_on_stack xmm_regs_used - 16*cpuflag(avx512) - 8
    %if %%xmm_regs_on_stack > 0
...
%macro WIN64_RESTORE_XMM_INTERNAL 1
    %assign %%pad_size 0
    %assign xmm_regs_used xmm_regs_used - 16*cpuflag(avx512)
    %if xmm_regs_used > 8
...
%define has_epilogue regs_used > 7 || stack_size > 0 || xmm_regs_used > 6+16*cpuflag(avx512) || \
                     (mmsize > 16 && (xmm_regs_used > 16 || notcpuflag(avx512)))

%macro RET 0
    WIN64_RESTORE_XMM_INTERNAL rsp
    POP_IF_USED 14, 13, 12, 11, 10, 9, 8, 7
    %if mmsize > 16 && (xmm_regs_used > 16 || notcpuflag(avx512))
        vzeroupper
    %endif
    AUTO_REP_RET
%endmacro
...
%define has_epilogue regs_used > 9 || stack_size > 0 || (mmsize > 16 && (xmm_regs_used > 16 || notcpuflag(avx512)))
...
%define has_epilogue regs_used > 3 || stack_size > 0 || mmsize > 16
...
%assign cpuflags_avx512   (1<<16)| cpuflags_avx2 ; F, CD, BW, DQ, VL

%assign cpuflags_cache32  (1<<17)
%assign cpuflags_cache64  (1<<18)
%assign cpuflags_slowctz  (1<<19)
%assign cpuflags_lzcnt    (1<<20)
...
%macro INIT_XMM 0-1+
    ...
    INIT_CPUFLAGS %1
    %if WIN64
        ; Swap callee-saved registers 6-15 with volatile registers 22-31
...
EVEX_INSTR vrcpps,         vrcp14ps,  1 ; AVX-512 versions have higher precision
EVEX_INSTR vrcpss,         vrcp14ss,  1
EVEX_INSTR vsqrtpd,        vsqrt14pd, 1
EVEX_INSTR vsqrtps,        vsqrt14ps, 1
EVEX_INSTR vsqrtsd,        vsqrt14sd, 1
EVEX_INSTR vsqrtss,        vsqrt14ss, 1
